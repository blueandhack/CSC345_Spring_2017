...
public class Proj05Main {
	public static void main(String[] args) throws IOException {
		...
		while (in.hasNext()) {
			...
			switch (cmd) {
			case "vertex":
				handleVertex(in, verts, names);
				break;
			case "edge":
				handleEdge(in, verts, names);
				break;
			case "reachable":
				handleReachable(in, verts, names);
				break;
			case "dijkstra":
				handleDijkstra(in, verts, names);
				break;
		...
		/*
		 * after the input file has been consumed, we print out the .dot file.
		 */
		Proj05Vertex[] arr = new Proj05Vertex[verts.size()];
		Proj05StudentCode.printDotFile(verts.toArray(arr), basename + "-student.dot");

		/*
		 * We do a final printout, to convirm that the program didn't crash.
		 */
		System.out.println("Testcase completed.");
	}

	public static void handleVertex(Scanner in, ArrayList<Proj05Vertex> verts, HashMap<String, Integer> names) {
		String name = in.next();
	...
	public static void handleEdge(Scanner in, ArrayList<Proj05Vertex> verts, HashMap<String, Integer> names) {
		...
		if (from_I == null || to_I == null)
			throw new IllegalArgumentException(String.format(
					"Either the from or two of an edge didn't exist.  from='%s' to='%s'\n", from_name, to_name));
		...
		if (from.containsEdgeTo(to))
			throw new IllegalArgumentException(
					String.format("The 'from' vertex already has an edge to the 'to' vertex.  from='%s' to='%s'\n",
							from_name, to_name));
	...
	public static void handleReachable(Scanner in, ArrayList<Proj05Vertex> verts, HashMap<String, Integer> names) {
		String from_name = in.next();
		String to_name = in.next();
		...
		if (from_I == null || to_I == null)
			throw new IllegalArgumentException(
					String.format("dijkstra: Either the from or two of an edge didn't exist.  from='%s' to='%s'\n",
							from_name, to_name));
	...
	public static void handleDijkstra(Scanner in, ArrayList<Proj05Vertex> verts, HashMap<String, Integer> names) {
		String from_name = in.next();
		String to_name = in.next();
