...
public class Proj04Timer {
	public static void main(String[] args) {
		...
		try {
			...
			System.err.println(
					"ERROR: Could not part the first 3 parameters.  Give 3 integers (start,end,step) and 1 string (operation to test).");
			System.exit(1);
		}
	...
	public static void runTest(String type, int size) {
		...
		for (int cls = 0; cls < 7; cls++)
			for (int i = 0; i < COUNT; i++)
				switch (cls) {
				case 0:
					empties[cls][i] = new UnsortedArray();
					break;
				case 1:
					empties[cls][i] = new SortedArray();
					break;
				case 2:
					empties[cls][i] = new BST();
					break;
				case 3:
					empties[cls][i] = new AVLTree();
					break;
				case 4:
					empties[cls][i] = new HashTable();
					break;
				case 5:
					empties[cls][i] = new HashMap_Wrapper();
					break;
				case 6:
					empties[cls][i] = new TreeMap_Wrapper();
					break;
				}
		...
		for (int i = 0; i < 7; i++) {
			double time = runTestSet(empties[i], type, keys, vals);
	...
	public static float runTestSet(Proj04Dictionary[] empties, String type, int[][] keys, String[][] vals) {
		// sanity checking
		if (empties.length <= 2 || empties.length != keys.length || empties.length != vals.length) {
	...
	// with timing.
	public static long runOneTest(Proj04Dictionary dict, String type, int[] keys, String[] vals) {
		// sanity checking
	...
	public static void doTest(Proj04Dictionary dict, String type, int[] keys, String[] vals) {
		// sanity checks
